...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_processors', default=3, type=int)
    parser.add_argument('--data_path', default="/mnt/data/group/text2img_data/data_font_en/aesthetics_tar_5/*", type=str) 
    parser.add_argument('--output_path', default="/mnt/data/group/text2img_data/data_font_en/ae", type=str) 
    ...
    ocr = PaddleOCR(use_angle_cls=True, lang="en", det_model_dir="/mnt/data/group/majian/glyphdraw2/model_ocr/en_PP-OCRv3_det_infer", \
                    rec_model_dir="/mnt/data/group/majian/glyphdraw2/model_ocr/en_PP-OCRv4_rec_infer",  \
    ...
    def collate_fn(examples):
        ...
        return { "jpg": jpg, "key": key,"txt": txt}

    output_path = f"{output_path}/{process_id}"
    os.makedirs(output_path, exist_ok=True)

    tar_list = sorted(glob.glob(data_path))
    curr_tar_list = np.array_split(tar_list, num_processors)[process_id]
    rs = MultiProcessingReadingService(num_workers=1)
    print("nums of data:",len(curr_tar_list))
    index = 0
    num_per_dir = 10000
    pattern = os.path.join(output_path, f"%05d.tar")
    sink = webdataset.ShardWriter(pattern, maxsize=int(6e9), maxcount=int(num_per_dir))

    for tar_index,tar_name in tqdm.tqdm(enumerate(curr_tar_list)):
        dataset = FileOpener([tar_name], mode="b").load_from_tar().map(decode).webdataset().filter(filter_fn=filter_resolution, input_col=".jpg").batch(batch_size).collate(collate_fn=collate_fn)
        ...
        for obj in dl:
            with torch.no_grad():
                ...
                for j,img in enumerate(jpgs):
                    ...
                    try:
                        ...
                        print("error",img_np)
                        continue
                    if len(rec_res)<1 or len(rec_res)>20:continue 
                    qualified_res = []
                    chars = []
                    box_no = []
                    text_no = []
                    for box, rec1 in zip(dt_boxes, rec_res):
                        ...
                        if conf_score < 0.6:continue
                        center = np.mean(box, axis=0)
                        area = Polygon(box).area/(img_h*img_w)
                        if contains_alpha(ocr_text):
                        ...
                        if "@" in ocr_text or "有限" in ocr_text or "com" in ocr_text:
                            ...
                            continue
                        if conf_score > 0.75 and len(ocr_text) <= 100 and char_areas > 0.0006:
                            qualified_res.append((ocr_text, corners_text))
                            chars.append(ocr_text)
                        else:
                            box_no.append(box)
                            text_no.append((ocr_text,char_areas))
                    if len(box_no)>7:continue
                    if box_no and qualified_res:
            ...
            for i in range(len(jpgs_new)):
                ...
                sink.write({
                    "__key__": key,
                    "txt": generated_text[i],
                    "jpg": jpgs_new[i],
                    "text": " && ".join(fonts[i]),
                    "json": {"caption_en": generated_text[i], "caption_zh": txts_new[i], "font": font_infors[i]}})
                index += 1
    dl.shutdown()
    sink.close()